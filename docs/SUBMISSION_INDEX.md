# SautiForm Africa — Challenge Submission Package

This directory contains the written competition package for the Sahara CodeSwitch Africa Main
Challenge.

## Core submission documents

1. [Solution Description](SUBMISSION_SOLUTION_DESCRIPTION.md)
2. [Submission Benchmark Report PDF](SautiForm_Africa_Benchmark_Report.pdf) - final 3-page judge-facing report
3. [Full Benchmark Report](BENCHMARK_REPORT.md)
4. [Ethics & Inclusion Note](ETHICS_AND_INCLUSION_NOTE.md)
5. [Demo Video Script and Recording Plan](DEMO_VIDEO_SCRIPT.md)
6. [Submission Checklist](SUBMISSION_CHECKLIST.md)

## Supporting technical evidence

- [Final Held-out Evaluation](FINAL_HELDOUT_EVALUATION.md)
- [Benchmark Protocol](BENCHMARK_PROTOCOL.md)
- [Held-out Collection Plan](HELDOUT_COLLECTION_PLAN.md)
- [Held-out Freeze](HELDOUT_FREEZE.md)
- [Sahara Setup](SAHARA_SETUP.md)
- [Omnilingual Setup](OMNILINGUAL_SETUP.md)
- [Responsible AI](RESPONSIBLE_AI.md)
- [Phase 2 Specification](PHASE_2_SPECIFICATION.md)

The public submission material contains aggregate benchmark evidence only. Private audio, API keys
and per-clip held-out outputs remain outside the repository.
